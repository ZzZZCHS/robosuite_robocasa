...
class MjRenderContext:
    ...
    def read_pixels(self, width, height, depth=False, segmentation=False):
        ...
        if segmentation:
            ...
            # ret_img = (ret_img[:, :, 1:2].repeat(3, axis=2)/ret_img[:,:,1].max()*255).astype(np.uint8)
        
        
        # new_img = (ret_img[:, :, 0:1].repeat(3, axis=2)/ret_img[:,:,0].max()*255).astype(np.uint8)
        # Image.fromarray(new_img).save('tmp.jpg')
        # breakpoint()
...
class MjSimState:
    ...
    def from_flattened(cls, array, sim):
        ...
        """
        try:
            if sim.model.nq + sim.model.nv + 1 > array.shape[0]:
                assert (sim.model.nq + sim.model.nv + 1 - array.shape[0]) % 13 == 0
                extra_num = (sim.model.nq + sim.model.nv + 1 - array.shape[0]) // 13
                idx_time = 0
                idx_qpos = idx_time + 1
                idx_qvel = idx_qpos + sim.model.nq - extra_num * 7
                time = array[idx_time]
                qpos = array[idx_qpos:idx_qvel]
                qvel = array[idx_qvel:]
            else:
                idx_time = 0
                idx_qpos = idx_time + 1
...
class MjSim:
    ...
    def set_state_from_flattened(self, value):
        ...
        self.data.qpos[:state.qpos.shape[0]] = state.qpos
        self.data.qvel[:state.qvel.shape[0]] = state.qvel
        # if state.qpos.shape[0] < self.data.qpos.shape[0]:
        #     tmp_st, _ = self.model.get_joint_qpos_addr('distr_counter_joint0')
        #     ori_pos = self.data.qpos[tmp_st:tmp_st+3].copy()
        #     ori_quat = self.data.qpos[tmp_st+3:tmp_st+7]
        #     tmp_st, _ = self.model.get_joint_qvel_addr('distr_counter_joint0')
        #     ori_qvel = self.data.qpos[tmp_st:tmp_st+6]
        #     cur_qpos_idx = self.data.qpos.shape[0] - 7
        #     cur_qvel_idx = self.data.qvel.shape[0] - 6
        #     tmp_idx = 1
        #     cur_pos = ori_pos
        #     cur_pos[-1] += 0.05
        #     while cur_qpos_idx >= state.qpos.shape[0]:
        #         pos_delta = random.uniform(0.15, 0.2)
        #         cur_pos[tmp_idx % 2] += pos_delta * (1 if tmp_idx % 4 == 2 else -1)
        #         self.data.qpos[cur_qpos_idx:cur_qpos_idx+3] = cur_pos
        #         self.data.qpos[cur_qpos_idx+3:cur_qpos_idx+7] = ori_quat
        #         self.data.qvel[cur_qvel_idx:cur_qvel_idx+6] = ori_qvel
        #         tmp_idx += 1
        #         cur_qpos_idx -= 7
        #         cur_qvel_idx -= 6
